...
public class MigrationTest extends BaseTest {
    ...
    //@Test
    public void testMigration_Valid24Words_CorrectValuesReturn()
            throws Throwable {
        List<String[]> twentyFourWords = new ArrayList<String[]>() {{
            add("abuse tooth riot whale dance dawn armor patch tube sugar edit clean guilt person lake height tilt wall prosper episode produce spy artist account"
                    .split(" "));
            add("accident syrup inquiry you clutch liquid fame upset joke glow best school repeat birth library combine access camera organ trial crazy jeans lizard science"
                    .split(" "));
        }};

        for (String[] words : twentyFourWords) {
            Pair<Account, List<String>> pair = await(
                    callback -> Migration.migrate(
                            words,
                            callback
                    ),
                    TIMEOUT
    ...
    public void testMigration_Invalid24Words_ErrorIsThrow() throws Throwable {
        ...
        List<String[]> twentyFourWords = new ArrayList<String[]>() {{
            ...
            add(new String[]{});
            add(new String[24]);
            add(("during kingdom crew atom practice brisk weird document eager artwork ride then")
                    .split(" "));
        }};
        for (String[] words : twentyFourWords) {
            expectedException.expect(ValidateException.class);
            await(
                    (Callable1<Pair<Account, List<String>>>) callback -> Migration
                            .migrate(
                                    words,
                                    callback
                            ),
